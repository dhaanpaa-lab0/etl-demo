...
- Support multiple database backends (PostgreSQL, MySQL, DuckDB)
- Read and write files in multiple formats (CSV, Excel, JSON, Parquet, XML)
- Work with pandas DataFrames for data transformation and database operations
...
- **etl/sys.py**
    - SysFolderType: enum defining standard ETL folders (in, out, logs, tmp, dat, ctl).
...
- **etl/core.py**
    - EtlDbConfig: builds a SQLAlchemy connection URI from env vars (PG_*). Supports named environments (e.g., PG_DEV_*)
      and multiple database backends (PostgreSQL, MySQL, DuckDB).
...
- **etl/fil.py**
    - FileSource: reads and writes files in various formats using pandas. Supports CSV, Excel, JSON, Parquet, and XML.
- **etl/cntrl.py**
    - EtlControl: database access layer with methods for querying control tables and DataFrame operations:
        - get_file_sources(): queries ctl_file_sources table
        - read_sql_as_dataframe(sql): executes SQL and returns pandas DataFrame
        - write_dataframe_to_sql_overwrite(df, table_name): writes DataFrame to table (replace mode)
        - write_dataframe_to_sql_append(df, table_name): writes DataFrame to table (append mode)
- **main.py**
...
- Python 3.12+
- A database instance to connect to:
    - PostgreSQL (server-based)
    - MySQL (server-based)
    - DuckDB (local/embedded, file-based or in-memory)

## Project dependencies (via pyproject.toml)

- sqlalchemy>=2.0 (ORM and database toolkit)
- psycopg>=3 (PostgreSQL driver, used by SQLAlchemy URI: postgresql+psycopg)
- pymysql>=1.1.0 (MySQL driver, used by SQLAlchemy URI: mysql+pymysql)
- duckdb>=1.1.3 (embedded database engine)
- duckdb-engine>=0.13.2 (SQLAlchemy dialect for DuckDB)
- python-dotenv (loads .env files)
- alembic (database migrations)
- pandas>=2.3.3 (data manipulation, required for FileSource and DataFrame operations)
...
The connection URI will be `mysql+pymysql://USER:PASS@HOST:PORT/DB`
...
- **etl/sys.py** - System enumerations (folder types, file types)
- **etl/core.py** - Config, engine factory, and folder management
- **etl/fil.py** - File I/O operations with pandas
- **etl/cntrl.py** - Control-table data access and DataFrame operations
...
- Connection errors (PostgreSQL): Verify PG_* variables and that your database is reachable. Confirm your driver is
  postgresql+psycopg and psycopg>=3 is installed.
- Connection errors (DuckDB): Verify that PG_TYPE=duckdb and PG_DB is set to a valid file path or :memory:. Ensure
  duckdb>=1.1.3 and duckdb-engine>=0.13.2 are installed.
- Missing table ctl_file_sources: Run alembic upgrade head to apply migrations. Note: Alembic migrations are primarily
  designed for PostgreSQL; DuckDB users may need to adapt or create tables manually.
- Permission issues creating folders: The app creates in, out, logs, tmp, dat, ctl under the working directory. Run from
  a writable location or adjust EtlEnvironment(sys_root).
- DuckDB file permissions: If using a file-based DuckDB, ensure the directory where the database file will be created is
  writable.
